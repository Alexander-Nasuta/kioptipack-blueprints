...
from kio.ml_lifecycle_subjects_name import DB_SAVE_MANY_RAW_DATAPOINTS_SUBJECT, \
    DB_UPSERT_MANY_PROCESSED_DATAPOINTS_SUBJECT, DB_GET_ALL_RAW_DATA_SUBJECT, DB_GET_PROCESSED_DATA_PAGE_SUBJECT, \
    DB_GET_PROCESSED_DATA_COUNT_SUBJECT, DB_GET_LABELED_DATASET_SUBJECT, DB_GET_ALL_PROCESSED_DATA_SUBJECT
...
class MongoDatabaseService(FastIoTService):
    ...
    _MONGO_DB = "fiot-pytorch-training-blueprint" # name of the mongo database
    ...
    async def get_all_raw_data(self, _: str, __: Thing) -> Thing:
        ...
        :param _: The topic of the message. This is not used in this method.
        :type _: str
        :param __: The message that contains the request to get all raw data from the database.
        :type __: Thing
        :return: A Thing object that contains the result of the operation. This is either an acknowledgement or an error.
    ...
    async def db_save_upsert_processed_datapoints(self, _: str, msg: Thing) -> Thing:
        ...
        :param _: Das Thema der Nachricht. Wird in dieser Methode nicht verwendet.
        :type _: str
        :param msg: Die Nachricht, die die zu upsertenden verarbeiteten Datenpunkte enthält.
        :type msg: Thing
        :return: Ein Thing-Objekt, das das Ergebnis der Operation enthält. Dies ist entweder eine Bestätigung oder ein Fehler.
                 Bestätigungen enthalten die Anzahl der verarbeiteten Datenpunkte, die in die Datenbank upserted wurden.
        :rtype: Thing
        """
    ...
    @reply(DB_GET_ALL_PROCESSED_DATA_SUBJECT)
    async def get_labeled_dataset(self, _: str, __: Thing) -> Thing:
        self._logger.info("Received request to get labeled dataset.")

        try:
            labeled_data_entries = self._processed_data_collection.find()
            labeled_data_entries = [dict(data) for data in labeled_data_entries]

            # the native mongo ID is not serializable to json
            # so we convert it to a string
            for data in labeled_data_entries:
                # uncomment if you want to keep the _id field as string
                # data["_id"] = str(data["_id"])

                # comment out if you want to keep the _id field
                # drop th _id column for the labeled dataset
                del data["_id"]
