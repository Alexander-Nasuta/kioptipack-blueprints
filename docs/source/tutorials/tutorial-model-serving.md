...
```{index} double: Tutorial; MLflow Model Serving
```

```{raw} html
<span class="index-entry">Tutorial</span>
<span class="index-entry">MLflow Model Serving</span>
```

This tutorial demonstrates how to set up a FastIoT project that serves machine learning models using MLflow.
It includes services for data storage in MongoDB, data processing, model training with PyTorch,
and model serving with MLflow.
...
4. **Model Serving Service**: Code snippet for the Model Serving Service.
5. **Run the Services**: Instructions to run the services and verify their functionality.
6. **Verify Model Serving**: Steps to check the served models and predictions.
...
| **Create a Model Serving Service**  | Service to serve trained models via MLflow                      | `fiot create new-service model_serving`                                                      |
| **Create a ML consumer Service**    | Service to consume models from MLflow                           | `fiot create new-service ml_consumer`                                                        |
